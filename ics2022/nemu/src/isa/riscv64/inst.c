...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("???????????? ????? 110 ????? 0010011", ori, I, R(rd) = src1 | imm);  // or
  ...
  INSTPAT("???????????? ????? 000 ????? 0000011", lb, I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));  // 8bits sign-extend
  ...
  INSTPAT("???????????? ????? 110 ????? 0000011", lwu, I, R(rd) = Mr(src1 + imm, 4)); // 32bits zero-extend
  ...
  INSTPAT("???????????? ????? 000 ????? 1100111", jalr, I, R(rd) = s->snpc; s->dnpc = ((src1 + imm) & ~1);
#ifdef CONFIG_FTRACE
          if (rd == 1) Call_print(s->pc, s->dnpc); if (rs1 == 1) Ret_print(s->pc, s->dnpc)
#endif
  );
  // src1 和 imm 都看作是无符号数！
  INSTPAT("???????????? ????? 011 ????? 0010011", sltiu, I, R(rd) = (src1 < imm ? 1 : 0));                  // set less than imm (unsigned)
  INSTPAT("???????????? ????? 010 ????? 0010011", slti, I, R(rd) = ((sword_t)src1 < (sword_t)imm ? 1 : 0)); // set less than imm (signed)
  ...
  INSTPAT("000000 ?????? ????? 001 ????? 0010011", slli, I, R(rd) = (src1 << BITS(imm, 5, 0))); // 逻辑左移 (using immediate)
  ...
  INSTPAT("010000 ?????? ????? 101 ????? 0010011", srai, I, R(rd) = (sword_t)src1 >> BITS(imm, 5, 0)); // 算术右移（先符号扩展，然后右移）
...
#ifdef CONFIG_FTRACE
          if (rd == 1) Call_print(s->pc, s->dnpc);
#endif
  ); // 无条件跳转
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 0110011", rem, R, R(rd) = (sword_t)src1 % src2); // % signed
  INSTPAT("0000001 ????? ????? 111 ????? 0110011", remu, R, R(rd) = src1 % src2);         // % unsigned
  INSTPAT("0000001 ????? ????? 100 ????? 0110011", div, R, R(rd) = (sword_t)src1 / src2); // / signed
  ...
  // INSTPAT("0000001 ????? ????? 100 ????? 0111011", divw, R, R(rd) = SEXT((sword_t)BITS(src1, 31, 0) / BITS(src2, 31, 0), 32)); // / signed
  INSTPAT("0000001 ????? ????? 100 ????? 0111011", divw, R, R(rd) = SEXT((int32_t)BITS(src1, 31, 0) / (int32_t)BITS(src2, 31, 0), 32)); // / signed
  INSTPAT("0000001 ????? ????? 101 ????? 0111011", divuw, R, R(rd) = SEXT(BITS(src1, 31, 0) / BITS(src2, 31, 0), 32));                  // / unsigned

  // INSTPAT("0000001 ????? ????? 110 ????? 0111011", remw, R, R(rd) = SEXT((sword_t)BITS(src1, 31, 0) % BITS(src2, 31, 0), 32)); // % signed
  INSTPAT("0000001 ????? ????? 110 ????? 0111011", remw, R, R(rd) = SEXT((int32_t)BITS(src1, 31, 0) % (int32_t)BITS(src2, 31, 0), 32)); // % signed
  INSTPAT("0000001 ????? ????? 111 ????? 0111011", remuw, R, R(rd) = SEXT(BITS(src1, 31, 0) % BITS(src2, 31, 0), 32));                  // % unsigned

  INSTPAT("0000001 ????? ????? 101 ????? 0110011", divu, R, R(rd) = src1 / src2); // unsigned 64 bit /

  INSTPAT("0000000 ????? ????? 101 ????? 0110011", srl, R, R(rd) = (src1 >> BITS(src2, 5, 0))); // logistic right left (using register)
  INSTPAT("0000000 ????? ????? 001 ????? 0110011", sll, R, R(rd) = (src1 << BITS(src2, 5, 0))); // logistic shift left (using register)
